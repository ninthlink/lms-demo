...
Changelog v3.7.4
	- Added new option: singlePageNav
		• Option to have the navigation still up when on a single page of content
	- init():
		• Added the if-statement to check for if singlePageNav is true, if so then we don't
		  need to set clickHandler, but rather $.publish('slider/goto-assessment') so that
		  the outside JS can handle the event. Also, passes out the option value it self to
		  be used outside.
	- createNav(): 
		• Added variable to reference "self"
		• Added OR option into if-statement for check of if singlePageNav is enabled/true,
		  then we can also still fade in the slider controls.

Changelog v3.7.5
	- init():
		• Added new if-statement to check for if singlePageNav is true, and if so add to
		  the html element, a class of 'single-page-nav'

Changelog v3.7.6
	- Removed trailing "," commas, that trigger issues in IE7

Changelog v3.7.7
	- setHeight():
		• Removed variables: padding, slidePos, finHeight
		• Added variables: footerHeight, topbarHeight
		• The "slides" min-height value is now set to the calculated value of the window/
		  viewport's height minus the combined heights of the footer, and top-bar.
...
Options: 
	...
	- singlePageNav: [default = false ] - Boolean: This checks and sets up the slider navigation even though there is only
...
;(function($, window, document, undefined) {
	...
	var methods  = { 
		...
		init: function( options, elem ) {
			...
			//add class of '.single-slide-nav' to html tag, so can targer specifically, 
			//if there is only a single slide of content.
			if(self.options.singlePageNav) { $html.addClass('single-page-nav'); }

			//attach the click events, and main run the main interactions of the plugin
			$('a.nav-arrow').on('click', function(e) {
				if(!self.options.singlePageNav) {
					self.setClick.call(self, e, this);
				} else {
					$.publish('slider/goto-assessment', [self.options.singlePageNav]);
				}
			});
		...
		setHeight: function() {
			...
			$window.resize(function() { 
				//retrieve the current window/viewport's height
				winHeight = $window.height(); 
				
				//set the minimum height of the slides
				attributes.$slides.css('min-height', (winHeight - (footerHeight + topbarHeight)) );

				//update the "left" property to slide over
				attributes.containWidth = self.$elem.outerWidth();	
			});
		...
		createNav: function() {
			...
			if(attributes.storage.length > 1 || self.options.singlePageNav) {
