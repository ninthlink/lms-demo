...
Options: 
	...
	- linearNav:  [ default = false ] --- Boolean: to force linear navigati n 
	- linearLoop: [ default = true ] ---- Boolean: to enable/disable the user loop back to front and 
	             						  vice-versa. You must currently have "linearNav" enable for this
...
;(function($, window, document, undefined) {
	...
	var methods  = { 
		...
		linearNavInit: function ( elem ) {
			...
			attributes.iOSextra = (navigator.userAgent.match(/(iPhone)/)) ? 60 : 0;
		...
		//disable the navigation at first, and remove click event
		setNav: function() {
			var self = this;
			self.disableNav(self.nav.$arrows);
		},
		...
		checkSlide: function() {
			...
			if( $slide.hasClass('slide-first') ) {
				...
				if($slideDOM.visitCounter && $slideDOM.slideComplete) {
					self.enableNav(self.nav.$next);
				}
			}
		},
