...
function parseXML(xml) {
	...
	/*-------------------------------------------------------------------------
	Setting RTL attribute
	--------------------------------------------------------------------------*/
 	//Checks if the language is a right-to-left (RTL) language, and if so then add the 'dir' attribute 
 	//with value of 'rtl' to set the browser page accordingly for those languages
 	if(right == "true") {
 		$('html').attr({ dir: 'rtl'});
 	}

	/*-------------------------------------------------------------------------
	Concatinating language prefix to video files
	So that it points to the right one when the language select comes up. 
	-------------------------------------------------------------------------*/
	$('video').each(function() {
		var $this = $(this);
		var currentSrc = $this.children('source').attr('src');
		currentSrc = currentSrc.replace('/en/', '/'+language+'/');
		$this.children('source').attr('src', currentSrc);
		$this[0].src = currentSrc;
		//$(this)[0].load();
	});

	/*--------------------------------------------------------------------------
	Change the images with class of .img-change to the correct language version 
	--------------------------------------------------------------------------*/
	//change the images 
	$('.img-change').each(function() {
		var $this = $(this);
		//change img-change that is using "src" attribute
		var img_src = $this.attr('src');
		if(img_src != undefined) {
			img_src = img_src.replace('/en/', '/'+language+'/');
		...
		if($this[0].nodeName.toLowerCase() == 'video') {
			//$(window).load(function() {
				...
				// console.log($img_tag);
				// console.log(subimg_src);

				if(subimg_src != undefined) {
					subimg_src = subimg_src.replace('/en/', '/'+language+'/');
	...
	// console.time('timer');
	for(i in sections) {
		if(sections[i] !== 'container-loader') {
	...
	//set the bouquet
	// setBouquetText(sections['s1splash'], 'dots_popup', 'bouquet', 'mobile-bouquet');
...
/*---------------------------------------------------------------------------------------------------------------------------
Finds the text from the XML, and updates the text tooltip text as well as the mobile version of the course

@note
	- Currently targets Foundation Framework's tooltips ONLY!
	- CDATA makes some issues with it not rendered, but seems to be rendered correctly with foundations	

@param 
	- parent ---- Parent XML node to be searched 
	- tagName --- Name of the element node to return
	- targetID -- ID of the target of where to put new text

@calls 
	- updateTags();
---------------------------------------------------------------------------------------------------------------------------*/
function setBouquetText(parent, tagName, targetID, mobileTargetID) {
	var dots   		 = $(parent).find(tagName);
	var textContent  = [];  //holds the header text
	var length		 = dots.children().length;
	var tips 		 = $('span.tooltip');

	//pulls the text from the XML
	$(dots).find('text').each(function() {
	 	var $this = $(this);
	 	var text = $this.text();
	 	textContent.push(text);
	});

	//Combine the text together, and set it
	for(var i=0; i<length; i++) {		
		// Tablet/Desktop versions 
		$('#'+targetID+' div.hotspot'+(i+1)).attr('title', textContent[i]);

		// Set mobile version
		$('#'+mobileTargetID+' div.xml_bouquet'+(i+1)+' p').html(updateTags(textContent[i]));
	}
}
